                                                      ...
                                                      const bool use_low_precision_angles) const
{
    ...
    std::tie(first_coordinate, in_segment_length) = [&]() {
        ...
        const auto coordinate = extract_coordinate(from_node, via_eid, INVERT, turn_node);
        return std::make_pair(coordinate, length);
    }();
    ...
    for (const EdgeID onto_edge : node_based_graph.GetAdjacentEdgeRange(turn_node))
    {
        ...
        else
        {
            ...
            std::tie(third_coordinate, out_segment_length) = [&]() {
                ...
                const auto coordinate = extract_coordinate(turn_node, onto_edge, !INVERT, to_node);
                return std::make_pair(coordinate, length);
            }();
